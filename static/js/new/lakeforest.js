...
google.setOnLoadCallback(function(){
	$(function() {
		...
		var idArray = [],
				currentID=1298, currentView,												// used to identify current session and view
		...
		$('body').on('click', 'button#add-missed-runner', function(e) {
			...
			$.ajax({
				method: 'GET',
				url: 'api/athletes?registered_to_session=' + '1298' +
                     '&session!=' + '1298',
				headers: {Authorization: 'Bearer ' + localStorage.access_token},
				...
				success: function(data) {
					...
					if (data.length === 0) {
						...
						$('body').on('click', '#add-missed-runner-confirm', function(e) {
							...
							$.ajax({
								method: 'POST',
								url: 'api/sessions/'+'1298'+'/upload_results/',
		...
		$('body').on('click', 'button#download', function(e) {
            ...
            $('body').on('click', '#download-results-confirm', function(e) {
                ...
                $.ajax({
                    method: 'POST',
                    url: '/api/sessions/' + '1298' + '/export_results/',
                    headers: {
		...
		$('body').on('click', 'button#email', function(e) {
            ...
            $('body').on('click', '#email-results-confirm', function(e) {
                ...
                $.ajax({
                    method: 'POST',
                    url: '/api/sessions/' + '1298' + '/email_results/',
                    headers: {
                        Authorization: 'Bearer ' + localStorage.access_token
                    },
		...
		function drawIndividual() {
			...
			$.ajax({
				url: '/api/sessions/1298/individual_results/?gender='+gender+'&age_gte='+age_gte+'&age_lte='+age_lte+'&limit=150',
				headers: {Authorization: 'Bearer ' + localStorage.access_token},
				dataType: 'text',
				success: function(data) {
		...
		function drawTeam(){
			...
			spinner.spin(target);
			$.ajax({
				url: 'api/sessions/1298/team_results',
				headers: {Authorization: 'Bearer ' + localStorage.access_token},
				dataType: 'text',
				success: function(data) {
		...
		function createFullCSV(){
			$.ajax({
				url: '/api/sessions/1298/individual_results',
				headers: {Authorization: 'Bearer ' + localStorage.access_token},
				dataType: 'text',
				success: function(data) {
    	...
    	function drawIndividualSearch() {
			...
			$.ajax({
				url: '/api/athletes/?session=1298&search='+searchTerm,
				headers: {Authorization: 'Bearer ' + localStorage.access_token},
				dataType: 'text',
				success: function(data) {
					var results = $.parseJSON(data);
