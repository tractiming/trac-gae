...
class AthleteViewSet(viewsets.ModelViewSet):
    ...
    def completed_sessions(self, request, *args, **kwargs):
        ...
        records = PerformanceRecord.objects.filter(athlete__id=athlete.id)
        vo2_list = records.values_list('VO2', flat=True)
        # cast to int or leave as float?
        avg_vo2 = int(sum(vo2_list) / len(vo2_list)) if len(vo2_list) > 0 else 0 
        ...
        for session in sessions:
            ...
            avg_vo2_list = records.filter(
                    timingsession__id__lt=session.id).values_list('VO2', flat=True)
            avg_vo2 = (int(sum(avg_vo2_list) / len(avg_vo2_list))
                        if len(avg_vo2_list) > 0 else 0)
            # cast to int or leave as float?

            # calculate vo2 for this session
            vo2_list = records.filter(timingsession=session).values_list(
                    'VO2', flat=True)
            vo2 = int(sum(vo2_list) / len(vo2_list)) if len(vo2_list) > 0 else 0
            # cast to int or leave as float?
